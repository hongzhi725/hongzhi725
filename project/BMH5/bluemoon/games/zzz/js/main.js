		...
		switch(e) {
			case 0:
				...
				if(back == 'true') {
					...
					$('.p1').fadeOut();
					setTimeout(function() {
						$('.level_1').fadeIn(function() {
							A.attr('x', parseInt(A.position().left));
							A.attr('y', parseInt(A.position().top));
							B.attr('x', parseInt(B.position().left));
							B.attr('y', parseInt(B.position().top));
							C.attr('x', parseInt(C.position().left));
							C.attr('y', parseInt(C.position().top));
							$('#go').attr('isable', 'true');
						});
						$('.dec').fadeIn();
...
//			console.log('isRun：' + isRun);
	...
	function movecatch(object, callback) {
		...
		afterleft = $(object).position().left - $(object).attr('x');
		...
		move3d(c, left, 0, 500, function() {
			...
			move3d(c, left, top, 500, function() {
				...
				//一起上移
				move3d(c, left, 0, 500);
				move3d($(object), afterleft, height - $(object).attr('y'), 500, function() {
...
//		console.log('Second:go to ' + obj2_moveleft + ',' + obj2_movetop);
	...
	//		}, times * (delay + 300))
	//	}
	function rdmove(object, times, delay) {
		var time = 1;
		var i = object.length;
		isRun = true;
		isok = true;
